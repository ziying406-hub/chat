...
现有 React 客户端在不超过 768px 的屏幕下按单屏显示列表或详情；底部“通讯录／聊天／我的”只在根页面出现。个人中心根页面保留菜单，聊天、资料、收藏、群管理等详情有返回入口，资源不存在时也不形成导航死路。桌面多栏与业务权限不变。

本阶段完成的是响应式框架，不代表扫码、全局搜索、多语言或群举报已实现。真机键盘验收与生产发布状态见 [手机布局验证记录](mobile-layout-verification-2026-09-10.md)。
...
- 同账号多个 Web 浏览器同时在线；其他平台维持各平台单实例限制。Web 多登录由服务端补丁实现，仍保留凭据数量上限；不等同于多个 Web 均收到离线推送。
- 登录后建立 WebSocket 长连接
...
| 功能 | 说明 |
|---|---|
| 收藏消息 | 在聊天消息菜单中收藏文字、图片、语音、视频、文件或表情 |
| 分类浏览 | 按全部、文字、图片与视频、文件、语音筛选 |
| 收藏详情 | 点击条目查看发送者、日期时间和原始内容；媒体可直接预览或播放 |
| 多端读取 | 同一账号在不同浏览器或设备登录后可读取相同收藏 |
| 账户隔离 | 每个账号只可查看、删除自己的收藏，不与其他本机账号混用 |
| 离线回退 | 服务暂不可用时保留本机收藏；服务恢复后新收藏写入服务端 |
...
| 推送通知 | OpenIM 原生 FCM，用户授权且成功绑定后接收离线消息；关闭通知/退出撤销订阅，Chrome 无痕不支持 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| ClientMsgID | string | 原消息客户端 ID；同一用户内唯一 |
| SendID | string | 原消息发送者 ID |
| SenderName | string | 发送者展示名 |
| Content | string | 文本内容或媒体说明 |
| ContentType | int | OpenIM 消息类型 |
| Time | int | 原消息/收藏展示时间戳 |
| Kind | string | text、image、voice、video、file 或 face |
| MediaURL | string | 图片、语音、视频或文件的可访问地址 |
| Duration | int | 语音时长（秒，可选） |
...
- ✅ 通讯录（好友管理、群组管理）
- ✅ 一对一音视频通话
- PWA 安装 + 离线缓存；FCM 推送验收范围见 [生产验证记录](production-verification-2026-09-10.md)，不代表所有浏览器/系统均已通过
- ✅ 消息已读回执、撤回、转发
- ✅ 多语言支持
